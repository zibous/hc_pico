...
class PikoPVModel:
    ...
    def save_model(self, model: PVModelParameters):
        """Speichert die PV-Modell-Parameter."""
        try:
    ...
    def calculate_theoretical_power(self, steps: int = 96) -> dict:
        """
        Berechnet die theoretische PV-Momentanleistung und Tagesenergie
        basierend auf Sonnenstand und installierten Panelleistungen.
        """
        try:
            ...
            # Zeitgleichung (Equation of Time) & Längengrad-Korrektur
            ...
            # Modell-Parameter via Pydantic laden
            ...
            def pv_curve(t, shift):
                """Sinuskurve über den Tag mit harten Tagesgrenzen (Spike-Schutz)."""
            ...
            # Sunrise/Sunset als ISO-Strings
            sunrise = now.replace(
                hour=int(max(0, sunrise_h)),
                minute=int((max(0, sunrise_h) % 1) * 60),
                second=0,
                microsecond=0,
            )
            sunset = now.replace(
            ...
            return {
                ...
                "total": 0.0,
                "solar_time": 0.0,
                "solar_factor_ost": 0.0,
                "solar_factor_west": 0.0,
                "sunrise": None,
                "sunset": None,
